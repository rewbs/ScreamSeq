...
extension SignalGraphEditor {
  ...
  func buildSongOverview()->([SignalCanvasNode],[SignalCanvasEdge]){
    ...
    func edge(_ a:String,_ b:String,_ label:String,_ action:[String:Any]=[:],modulation:Bool=false,output:UInt32=0,input:UInt32=0){edges.append(SignalCanvasEdge(source:a,target:b,label:label,modulation:modulation,output:output,input:input));songConnections.append(action)}
    var row=0
    ...
    for (i,r) in (data["outputs"] as? [[String:Any]] ?? []).enumerated(){if let a=r["source"] as? String,let b=r["target"] as? String,visible.contains(a),visible.contains(b){let stage=display.last{songNodeBus[$0.id]==a && songNodeGraph[$0.id] != nil}?.id ?? a;edge(stage,b,"Graph out \(r["output"] ?? 1)",["kind":"graph-output","index":i],output:(r["output"] as? NSNumber)?.uint32Value ?? 1)}}
    for (i,r) in (mixer["sidechains"] as? [[String:Any]] ?? []).enumerated(){if let a=r["source"] as? String,let plugin=r["plugin"] as? String,let from=lastStage[a],songNodePlugin["plugin:\(plugin)"] != nil{edge(from,"plugin:\(plugin)","Sidechain \(r["input"] ?? 1)",["kind":"plugin-input","index":i],input:(r["input"] as? NSNumber)?.uint32Value ?? 1)}}
    for r in mixer["instruments"] as? [[String:Any]] ?? []{let plugin=r["plugin"] as? String ?? "";if rackPlugins.first(where:{$0["id"] as? String==plugin})?["isInstrument"] as? Bool != true,let target=r["target"] as? String,visible.contains(target),songNodePlugin["plugin:\(plugin)"] != nil{edge("plugin:\(plugin)",target,"Aux \(r["output"] ?? 0)",["kind":"plugin-output","plugin":plugin,"output":r["output"] ?? 0],output:(r["output"] as? NSNumber)?.uint32Value ?? 0)}}
    ...
    for i in display.indices {
      let id=display[i].id
      display[i].inputs=Set(edges.filter{$0.target==id}.map(\.input)+[UInt32(0)]).sorted().map{SignalCanvasPort(number:$0,label:"In \($0)")}
      display[i].outputs=Set(edges.filter{$0.source==id}.map(\.output)+[UInt32(0)]).sorted().map{SignalCanvasPort(number:$0,label:"Out \($0)")}
    }
  ...
  func connectSong(_ a:String,_ b:String){
    ...
    case "Graph sidechain":guard let port=Int(inputPort.stringValue)else{return};var routes=data["inputs"] as? [[String:Any]] ?? [];routes.append(["source":from,"target":to,"input":port,"gainDB":gain]);mutate("graph.routes.set",["inputs":routes])
