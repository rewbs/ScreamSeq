...
int main(){@autoreleasepool{try{
  ...
  NSString *automation=call(@"graph.node.add",@{@"graph":graph,@"kind":@"automation",@"name":@"Drawn motion"},true)[@"data"][@"node"];
  auto curveData=[&](){return call(@"graph.automation.get",@{@"graph":graph,@"node":automation,@"pattern":@0})[@"data"];};
  check([curveData()[@"points"] count]==0,"New graph curve must be empty");revision=session.automationRevision;
  NSDictionary *curve=@{@"graph":graph,@"node":automation,@"pattern":@0,@"points":@[@{@"position":@0,@"value":@0,@"curve":@"scripted",@"formula":@"mix(start,end,t^2)"},@{@"position":@255,@"value":@1,@"curve":@"step"}]};
  NSMutableDictionary *preview=[curve mutableCopy];preview[@"dryRun"]=@YES;call(@"graph.automation.set",preview,true);check([revision isEqual:session.automationRevision],"Graph curve preview mutates song");
  call(@"graph.automation.set",curve,true);check([curveData()[@"points"] count]==2,"Graph curve write missing");revision=session.automationRevision;call(@"graph.automation.set",curve,true);check([revision isEqual:session.automationRevision],"Identical graph curve creates Undo history");
  call(@"history.undo",@{@"domain":@"document"},true);check([curveData()[@"points"] count]==0,"Graph curve undo failed");call(@"history.redo",@{@"domain":@"document"},true);
  const auto instrument=[session addInstrument:1 error:&error];check(instrument>0,"Create instrument graph target");
  call(@"graph.instrument.assign",@{@"instrument":@(instrument),@"graph":graph,@"amount":@.3,@"wet":@.6},true);
  check([data()[@"instrumentAssignments"] count]==1,"Instrument graph assignment is missing");
  call(@"history.undo",@{@"domain":@"document"},true);check([data()[@"instrumentAssignments"] count]==0,"Instrument assignment undo failed");call(@"history.redo",@{@"domain":@"document"},true);
  ...
  reject(@"graph.automation.set",@{@"graph":graph,@"node":automation,@"pattern":@0,@"points":@[@{@"position":@0,@"value":@0},@{@"position":@0,@"value":@1}]});
  reject(@"graph.automation.set",@{@"graph":graph,@"node":node,@"pattern":@0,@"points":@[]});
  ...
  call(@"history.undo",@{@"domain":@"document"},true);check([data()[@"instrumentAssignments"] count]==0,"Undo failed to remove instrument graph transaction");call(@"history.redo",@{@"domain":@"document"},true);check([expected isEqual:data()],"Redo lost graph identities or values");
