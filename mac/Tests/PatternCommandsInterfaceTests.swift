...
extension InterfaceTests {
  static func patternCommandsChecks() throws -> PatternCommandPicker {
    ...
    picker.capture()
    try require(picker.filtered.count==3 && picker.table.selectedRow==1 && picker.parameter.stringValue=="D3", "Picker identifies extended command without losing its low parameter digit")
    ...
    try require(picker.captured.revisionToken=="original" && picker.status.stringValue.contains("Use cursor"),"Rejected edits require an explicit context refresh")
    ...
    model.replaceCell(0,0,with:[49,2,1,32,20,211]);picker.capture();picker.onRequest=nil
    return picker
  }
}
