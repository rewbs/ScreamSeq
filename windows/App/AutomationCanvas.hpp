...
struct AutomationCanvas {
  ...
  Point screen(double position,double value)const{return {viewport.x+float((position-start)/std::max(1.0,end-start))*viewport.w,viewport.y+float(1-value)*viewport.h};}
  double position(float x)const{return start+(x-viewport.x)*std::max(1.0,end-start)/std::max(1.0f,viewport.w);}
  double value(float y)const{return std::clamp(1-double(y-viewport.y)/std::max(1.0f,viewport.h),0.0,1.0);}
  ...
  void fit(unsigned rows){start=0;end=double(rows)*256;}
