...
protected:
  ...
  void drawControl(const DRAWITEMSTRUCT &d){RECT r=d.rcItem;const bool disabled=(d.itemState&ODS_DISABLED)!=0;SetDCBrushColor(d.hDC,RGB(35,49,63));FillRect(d.hDC,&r,reinterpret_cast<HBRUSH>(GetStockObject(DC_BRUSH)));SetBkMode(d.hDC,TRANSPARENT);SetTextColor(d.hDC,disabled?RGB(103,119,133):RGB(218,232,241));SelectObject(d.hDC,font_);std::wstring text;
  ...
  static LRESULT CALLBACK childProc(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR,DWORD_PTR context){auto &self=*reinterpret_cast<NativeToolWindow *>(context);if(m==WM_KEYDOWN||m==WM_SYSKEYDOWN)try{const bool ctrl=(GetKeyState(VK_CONTROL)&0x8000)!=0,shift=(GetKeyState(VK_SHIFT)&0x8000)!=0;if(w==VK_TAB){auto next=GetNextDlgTabItem(self.window_,h,shift);if(next)SetFocus(next);return 0;}if(self.key(w,ctrl,shift))return 0;}catch(const std::exception &e){self.error(e);return 0;}return DefSubclassProc(h,m,w,l);}
  static LRESULT CALLBACK proc(HWND h,UINT m,WPARAM w,LPARAM l){auto self=reinterpret_cast<NativeToolWindow *>(GetWindowLongPtrW(h,GWLP_USERDATA));if(m==WM_NCCREATE){self=static_cast<NativeToolWindow *>(reinterpret_cast<CREATESTRUCTW *>(l)->lpCreateParams);self->window_=h;SetWindowLongPtrW(h,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(self));}if(!self)return DefWindowProcW(h,m,w,l);
    try{switch(m){
      ...
      case WM_COMMAND:if(self->ready_){self->action(LOWORD(w),HIWORD(w));self->layout();self->requestPaint();}return 0;
...
public:
  ...
  void show(){ShowWindow(window_,SW_SHOWNORMAL);SetWindowPos(window_,HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE);requestPaint();}
