...
private:
  ...
  void inspect(bool audible){
    ...
    try{Json p={{"path",path}};if(audible)p["gainDB"]=number(gain);auto data=call(audible?"sample.library.preview":"sample.library.inspect",p);
  ...
  void action(int id,unsigned notification)override{
    if(setting_)return;if(id==close){hide();return;}if(id==stop){++generation_;call("sample.library.preview.stop");return;}if(pending_)return;
  ...
  void timer(UINT_PTR id)override{
    if(id!=1||!visible()||pending_)return;auto state=statusRead_();if(state_!=state){const bool changed=state_.is_null()||state_.value("libraryRevision",std::string{})!=state.value("libraryRevision",std::string{});state_=std::move(state);roots();if(changed){queued_=true;++generation_;}set(heading,L"Sample library / "+std::to_wstring(state_.at("count").get<unsigned>())+(state_.at("indexing").get<bool>()?L" / indexing…":L""));if(!state_.at("error").is_null())status(wide(state_.at("error").get<std::string>()));}
    if(queued_)load();SetTimer(window_,1,200,nullptr);layout();
  }
  bool key(WPARAM value,bool ctrl,bool)override{
    if(value==VK_ESCAPE){++generation_;call("sample.library.preview.stop");return true;}
    ...
    if(value==VK_RETURN){if(GetFocus()==controls_.at(files)){action(importSelection,BN_CLICKED);return true;}wchar_t type[32]{};GetClassNameW(GetFocus(),type,32);if(_wcsicmp(type,L"Button")==0){action(GetDlgCtrlID(GetFocus()),BN_CLICKED);return true;}}
    return false;
  }
  void layout()override{if(!ready_)return;const auto [w,h]=size();const auto left=190.f;place(heading,16,12,w-300,26);place(addFolder,w-276,12,100,26);place(removeFolder,w-168,12,78,26);place(rescan,w-82,12,66,26);
    ...
    for(int id:{search,tagSearch,root,tags,files,addFolder,removeFolder,rescan,chooseFiles,mapped,gain,autoPreview,previous,next})EnableWindow(controls_.at(id),!pending_);
  ...
  void paint(RenderSurface &s)override{const auto [w,h]=size();s.fill(0,0,w,h,0x18222d);const float x=452,y=h-146,width=std::max(8.f,w-x-180),height=29;s.fill(x,y,width,height,0x111b25);const auto mid=y+height/2;s.line(x,mid,x+width,mid,0x344a57);for(size_t i=0;i+1<peaks_.size();i+=2){const float at=x+float(i/2)*width/float(peaks_.size()/2);s.line(at,mid-peaks_[i]*height*.45f,at,mid-peaks_[i+1]*height*.45f,0x79d8c8);}}
public:
  SampleLibraryWindow(HWND owner,Request request,std::function<Json()> statusRead,std::function<Context()> context,std::function<void(const Json &,const std::string &)> imported,std::function<void(unsigned,const std::string &)> multisampleApplied):NativeToolWindow(owner),request_(std::move(request)),statusRead_(std::move(statusRead)),context_(std::move(context)),imported_(std::move(imported)){
  ...
  void hide()override{++generation_;call("sample.library.preview.stop");if(multisample_->visible())multisample_->hide();KillTimer(window_,1);NativeToolWindow::hide();}
