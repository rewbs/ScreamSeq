...
| Explicit disconnected mixer/plugin destinations | Actual-app mixer/graph API, native bus controls, reusable graph canvas, pattern curves and a modeless envelope bank integrated. The song routing overview adds bus/insert chains, graph stages, instrument outputs, sends, sidechains, graph ports and retained layout editing. See `MIXER_GRAPH_PROGRESS.md`, `GRAPH_EDITOR_PROGRESS.md`, `GRAPH_CURVES_PROGRESS.md`, `ENVELOPE_BANK_PROGRESS.md` and `SONG_ROUTING_PROGRESS.md`. | Graph command-lane controls, other automation/envelope editors and broader routing/long-session qualification. |
...
3. **Graph, mixer, automation and envelope editors.** Real rack activity,
   ...
   Complete graph command lanes, pattern-parameter automation, native instrument
