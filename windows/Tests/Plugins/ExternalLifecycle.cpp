...
static Json partitionProbe(const PluginState &state,unsigned rate) {
  ...
  for(unsigned block:{512u,512u,17u,128u,4096u}) {
    ...
    for(unsigned i=0;i<rate;++i)pcm[2*i]=pcm[2*i+1]=float(.125*std::sin(2*3.141592653589793*440*i/rate));
  ...
  try {
    ...
    for(const auto &descriptor:descriptors) {
      ...
      for(unsigned rate:{44100u,48000u,96000u}) {
        ...
        if(descriptor.instrument) check(plugin->midi(0x80,60,0),"Note-off rejected");
