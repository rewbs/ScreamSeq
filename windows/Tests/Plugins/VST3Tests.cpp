...
int main(int argc,char **argv){try{
 ...
 check(overflow->failure().reason&&std::string(overflow->failure().reason)=="vst3.input-parameter-capacity"&&overflow->failure().detail==7,"first overflow fault was overwritten by process failure");
 std::cout<<"PASS editor commit queue and overflow fault/silence\n";
 auto outputMode=symbol<void(*)(int)>(dll,"FixtureOutputMode");
 for(int mode=1;mode<=3;++mode){auto fresh=factory.create(effect,48000,true);outputMode(mode);pcm.fill(1);check(!fresh->process(pcm.data(),64,0,nullptr,0,{}),"vendor output overflow not faulted");for(int i=0;i<128;++i)expectPCM(pcm[i],0);check(fresh->failure().reason&&std::string(fresh->failure().reason)==(mode==1?"vst3.output-event-capacity":"vst3.output-parameter-capacity"),"missing vendor output fault detail");outputMode(0);}
